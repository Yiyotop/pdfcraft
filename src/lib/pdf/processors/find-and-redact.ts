...
export async function searchTextInPDF(
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      ...
      onProgress?.(
        pageProgress,
        `Searching page ${pageNum} of ${totalPages}...`,
      );
        ...
        for (const item of textContent.items) {
          ...
          for (const { pattern: searchPattern } of searchPatterns) {
            ...
            while ((match = searchPattern.exec(text)) !== null) {
              ...
              const height =
                Math.abs(textItem.height) || Math.abs(transform[0]) * 0.8;
...
export async function applyFindAndRedact(
  ...
  try {
    ...
    const selectedMatches =
      options.selectedMatchIds && options.selectedMatchIds.length > 0
    ...
    for (let i = 0; i < selectedMatches.length; i++) {
      ...
      onProgress?.(
        progress,
        `Redacting ${i + 1} of ${selectedMatches.length}...`,
      );
