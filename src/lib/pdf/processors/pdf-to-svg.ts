...
async function loadPdfjsLegacy() {
  const legacyLoaderModule = await import("../loader-legacy");
  return legacyLoaderModule.loadPdfjsLegacy();
}

async function loadSVGGraphics(): Promise<SVGGraphicsConstructor> {
  const legacyLoaderModule = await import("../loader-legacy");
  return legacyLoaderModule.loadSVGGraphics();
}
...
export class PDFToSVGProcessor extends BasePDFProcessor {
  ...
  async process(
    ...
    try {
      ...
      this.updateProgress(
        15,
        `Converting ${pagesToConvert.length} page(s) to SVG...`,
      );
      ...
      if (blobs.length === 1) {
        ...
        return this.createSuccessOutput(blobs[0], `${baseName}.svg`, {
          pageCount: 1,
          format: "svg",
        ...
        this.updateProgress(100, "Complete!");
        return this.createSuccessOutput(blobs, `${baseName}_pages.svg`, {
  ...
  private async renderPageToSVG(
    ...
    try {
      const svgString = await this.renderPageToVectorSVG(
        page,
        viewport,
        options,
      );
      ...
      console.warn(
        "Vector SVG rendering failed, falling back to canvas:",
        vectorError,
      );
