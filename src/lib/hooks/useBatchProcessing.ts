...
export function useBatchProcessing(
      ...
      prev.map((f) => {
        ...
        if (typeof updates.progress === "number") {
          const normalizedProgress = Math.max(
            0,
            Math.min(100, updates.progress),
          );
          next.progress = Math.max(f.progress, normalizedProgress);
        }
  ...
  const startProcessing = useCallback(
    async (
      ...
      // Capture current files snapshot from current state value
      ...
      const processFile = async (batchFile: BatchFile) => {
        ...
        try {
          const result = await processor(batchFile.file, (progress) => {
            const now = Date.now();
            const delta = progress - lastProgressValue;

            // Throttle frequent updates to reduce UI re-render overhead,
            // but always allow meaningful jumps and completion.
            if (
              progress >= 100 ||
              delta >= 1 ||
              now - lastProgressUpdateAt >= 120
            ) {
              lastProgressValue = Math.max(lastProgressValue, progress);
              lastProgressUpdateAt = now;
              updateFile(batchFile.id, { progress: lastProgressValue });
            }
          });
          ...
          onError?.(
            { ...batchFile, status: "error", error: error.message },
            error,
          );
        }
      };
    ...
    [maxConcurrent, files, updateFile, onFileComplete, onAllComplete, onError],
  );
  ...
  const downloadAsZip = useCallback(
    async (zipFilename = "batch_processed.zip") => {
      const completedFiles = files.filter(
        (f) => f.status === "completed" && f.result,
      );
  ...
  const overallProgress =
    files.length > 0 ? Math.round(totalProgress / files.length) : 0;
