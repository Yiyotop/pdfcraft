...
export function ToolSidebar({
  ...
  const formatToolId = useCallback((id: string): string => {
    return id
      .split("-")
  ...
  }, [categories, getToolName, searchQuery]);
  ...
  const handleDoubleClick = (tool: (typeof tools)[0]) => {
    ...
    window.dispatchEvent(
      new CustomEvent(WORKFLOW_TOOL_DROP_EVENT, {
        detail: {
          nodeData: createNodeData(tool),
          clientX,
          clientY,
      ...
      <div className="p-4 border-b border-[hsl(var(--color-border))] flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[hsl(var(--color-muted-foreground))] mt-1">
            {tWorkflow("dragToAdd") ||
              "Drag or double-click a tool to add it to the workflow"}
          </p>
        </div>
        <button
          onClick={onToggleCollapse}
