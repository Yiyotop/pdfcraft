...
} from "@/lib/pdf/processors/pdf-to-image";
    ...
    const getGridDimensions = (): [number, number] => {
      switch (layoutPreset) {
        ...
        case "3x3":
          return [3, 3];
        case "custom":
          return [customColumns, customRows];
        default:
          return [1, 1];
      }
    };
  ...
  return (
    <div className={`space-y-6 ${className}`.trim()}>
      ...
      <FileUploader
        ...
        description={
          tTools("pdfToImage.uploadDescription") ||
        ...
        <Card variant="outlined">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {["jpg", "jpeg", "webp"].includes(format) && (
              <div>
                <label className="block text-sm font-medium text-[hsl(var(--color-foreground))] mb-2">
                  {tTools("pdfToImage.quality") || "Quality"} (
            ...
            <div>
              ...
              <input
                ...
                placeholder={
                  tTools("pdfToImage.pageRangePlaceholder") || "e.g., 1-3, 5, 7"
                }
                disabled={isProcessing}
              ...
              <p className="text-xs text-[hsl(var(--color-muted-foreground))] mt-1">
                {tTools("pdfToImage.pageRangeHint") ||
          ...
          <div className="mt-4 pt-4 border-t border-[hsl(var(--color-border))]">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-4">
              {(
                [
                  ...
                  {
                    value: "custom",
                    label: tTools("pdfToImage.customLayout") || "Custom",
                    cols: customColumns,
                    rows: customRows,
                  },
                ] as const
            ...
            {layoutPreset !== "1x1" && (
              <label className="flex items-center gap-3 cursor-pointer mb-4">
                ...
                <span className="text-sm text-[hsl(var(--color-foreground))]">
                  {tTools("pdfToImage.skipFirstPage") ||
                    "Without first/cover page"}
                </span>
              </label>
            )}
                  ...
                  >
                    ...
                    >
                      {Array.from({
                        length:
                          layoutPreset === "custom"
                            ? customColumns * customRows
                            : parseInt(layoutPreset.split("x")[0]) *
                              parseInt(layoutPreset.split("x")[1]),
                      }).map((_, idx) => (
                        <div
                          key={idx}
                  ...
                  <div className="flex-1 text-sm text-[hsl(var(--color-muted-foreground))]">
                    ...
                    {skipFirstPage && (
                      <p className="mt-1 text-xs">
                        ℹ️{" "}
                        {tTools("pdfToImage.skipFirstPageHint") ||
                          "The first page (cover) will be rendered as a separate image"}
                      </p>
                    )}
                  </div>
                </div>
            ...
            <p className="text-xs text-[hsl(var(--color-muted-foreground))] mt-3">
              {tTools("pdfToImage.layoutHint") ||
                "Combine multiple PDF pages into a single image with the selected grid layout."}
            </p>
          </div>
        </Card>
      )}
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        {isMultipleImages && (
          <Button variant="secondary" size="lg" onClick={handleDownloadZip}>
            {tTools("pdfToImage.downloadZip") ||
              `Download All (${(result as Blob[]).length} images)`}
          </Button>
        )}
      </div>
      ...
      {isMultipleImages && (
        <Card variant="outlined" size="lg">
          <h3 className="text-lg font-medium text-[hsl(var(--color-foreground))] mb-4">
            {tTools("pdfToImage.previewTitle") || "Converted Images"} (
            {(result as Blob[]).length})
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {(result as Blob[]).map((blob, index) => (
              <div key={index} className="relative group">
                <div className="relative aspect-[3/4] rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] overflow-hidden bg-[hsl(var(--color-muted)/0.3)]">
                  <Image
      ...
      {status === "complete" && result && (
        ...
        >
          <p className="text-sm font-medium">
            {tTools("pdfToImage.successMessage") ||
              "PDF converted to images successfully! Click the download button to save your files."}
          </p>
        </div>
      )}
    </div>
