...
} from "@/lib/pdf/processors/extract-images";
  ...
  const handleExtract = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.metadata?.images) {
        ...
        setError(
          output.error?.message || "No images found in the selected PDF(s).",
        );
        setStatus("error");
      }
    } catch (err) {
  ...
  return (
    <div className={`space-y-6 ${className}`.trim()}>
      ...
      <FileUploader
        ...
        description={
          tTools("extractImages.uploadDescription") ||
          "Select PDF files to extract images from."
        }
      />
        ...
        <Card variant="outlined" size="lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-[hsl(var(--color-foreground))]">
              {tTools("extractImages.filesTitle") || "Selected Files"} (
              {files.length})
            </h3>
            <Button
              variant="ghost"
      ...
      {files.length > 0 && !isProcessing && extractedImages.length === 0 && (
        <Card variant="outlined" size="lg">
          ...
          <p className="text-sm text-[hsl(var(--color-muted-foreground))] mb-4">
            {tTools("extractImages.filterDescription") ||
              "Filter out small images (icons, decorations, etc.)"}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                value={minWidth}
                onChange={(e) =>
                  setMinWidth(Math.max(0, parseInt(e.target.value) || 0))
                }
            ...
            <div>
              ...
              <input
                ...
                value={minHeight}
                onChange={(e) =>
                  setMinHeight(Math.max(0, parseInt(e.target.value) || 0))
                }
                className="w-full px-3 py-2 border border-[hsl(var(--color-border))] rounded-[var(--radius-md)] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
              />
            </div>
      ...
      {extractedImages.length > 0 && (
        <Card variant="outlined" size="lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-[hsl(var(--color-foreground))]">
              {tTools("extractImages.extractedTitle") || "Extracted Images"} (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {extractedImages.map((image) => (
              ...
              >
                ...
                <div className="p-2">
                  <p
                    className="text-xs font-medium text-[hsl(var(--color-foreground))] truncate"
                    title={image.name}
                  >
                    {image.name}
                  </p>
      ...
      {status === "complete" && extractedImages.length > 0 && (
        ...
        >
          <p className="text-sm font-medium">
            {tTools("extractImages.successMessage") ||
              `Successfully extracted ${extractedImages.length} image(s). Click to download individual images or use "Download as ZIP".`}
          </p>
        </div>
      )}
