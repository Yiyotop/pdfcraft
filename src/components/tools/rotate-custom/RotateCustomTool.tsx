...
export function RotateCustomTool({ className = "" }: RotateCustomToolProps) {
  ...
  const updatePageRotation = useCallback(
    (pageNumber: number, angle: number) => {
      setPagePreviews((prev) =>
        prev.map((p) =>
          p.pageNumber === pageNumber ? { ...p, rotation: angle } : p,
        ),
      );
      setResult(null);
    },
   ...
   */
  const adjustPageRotation = useCallback(
    (pageNumber: number, amount: number) => {
      setPagePreviews((prev) =>
        prev.map((p) => {
      ...
      if (!cancelledRef.current) {
        setError(
          err instanceof Error
            ? err.message
            : t("errors.unknown") || "An unexpected error occurred.",
        );
        setStatus("error");
      }
    }
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center">
                <Button
                  ...
                  onClick={() =>
                    setBatchAngle((prev) => (parseFloat(prev) - 1).toString())
                  }
                >
                  -
                </Button>
      ...
      {status === "complete" && result && (
        ...
        >
          <p className="text-sm font-medium">
            Rotation completed successfully! Click the download button to save
            your file.
          </p>
        </div>
      )}
