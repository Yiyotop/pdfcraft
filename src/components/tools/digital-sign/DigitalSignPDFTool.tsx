...
export function DigitalSignPDFTool({ className = "" }: { className?: string }) {
  ...
  const parseCert = useCallback(
    async (file: File, pwd: string) => {
      try {
        const { parsePfxFile, parseCombinedPem, getCertificateInfo } =
          await import("@/lib/pdf/processors/digital-sign");
        ...
        if (expired) {
          setCertError(tTool("errorCertExpired"));
        }
      } catch (err) {
        setCertData(null);
        setCertInfo(null);
  ...
  const handleSign = useCallback(async () => {
    ...
    try {
      ...
      const { signPdf, getCertificateInfo } =
        await import("@/lib/pdf/processors/digital-sign");
  ...
  return (
    <div className={`space-y-6 ${className}`.trim()}>
      ...
      {error && (
        <div
          className="p-4 rounded-[var(--radius-md)] bg-red-50 border border-red-200 text-red-700 dark:bg-red-950 dark:border-red-800 dark:text-red-300"
          role="alert"
        >
      ...
      {pdfFile && (
        <Card variant="outlined">
          ...
          <div className="space-y-4">
            ...
            {certInfo && (
              <div
                className={`p-3 rounded-[var(--radius-sm)] space-y-1 ${certExpired ? "bg-yellow-50 border border-yellow-200 dark:bg-yellow-950 dark:border-yellow-800" : "bg-green-50 border border-green-200 dark:bg-green-950 dark:border-green-800"}`}
              >
                <p className="text-sm">
                ...
                <p className="text-sm">
                  ...
                  <span
                    className={`font-medium ${certExpired ? "text-red-600 dark:text-red-400" : "text-[hsl(var(--color-foreground))]"}`}
                  >
                    {formatDate(certInfo.validFrom)} —{" "}
                    {formatDate(certInfo.validTo)}
                  </span>
                </p>
                {certExpired && (
                  <p className="text-xs text-yellow-700 dark:text-yellow-300 mt-1">
                    {tTool("warningExpired")}
                  </p>
                )}
              </div>
            )}
      ...
      {certData && (
        <Card variant="outlined">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder={tTool("reasonPlaceholder")}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={tTool("locationPlaceholder")}
                  className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary))]"
                />
              </div>
            </div>
            <div>
              <label
                htmlFor="sign-contact"
                className="block text-sm font-medium text-[hsl(var(--color-foreground))] mb-1"
              >
                {tTool("contactInfoLabel")}
              </label>
              <input
                id="sign-contact"
                type="text"
                ...
                placeholder={tTool("contactInfoPlaceholder")}
                className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary))]"
              />
            </div>
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                type="checkbox"
                checked={enableVisible}
                onChange={(e) => setEnableVisible(e.target.checked)}
            ...
            {enableVisible && (
              <div className="space-y-3 pl-7">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div>
                    <label className="block text-xs text-[hsl(var(--color-muted-foreground))] mb-1">
                      {tTool("xPosition")}
                    </label>
                    <input
                      type="number"
                      ...
                      className="w-full px-2 py-1.5 text-sm rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-[hsl(var(--color-muted-foreground))] mb-1">
                ...
                <div>
                  <label className="block text-xs text-[hsl(var(--color-muted-foreground))] mb-1">
                    {tTool("signatureTextLabel")}
                  </label>
                  <input
                    type="text"
                    value={sigText}
                    onChange={(e) => setSigText(e.target.value)}
                    placeholder={tTool("signatureTextPlaceholder")}
                    className="w-full px-2 py-1.5 text-sm rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-[hsl(var(--color-muted-foreground))] mb-1">
                      {tTool("textColorLabel")}
                    </label>
                    <input
                      type="color"
                      value={sigTextColor}
                      onChange={(e) => setSigTextColor(e.target.value)}
                      className="w-full h-8 rounded-[var(--radius-sm)] border border-[hsl(var(--color-border))] cursor-pointer"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-[hsl(var(--color-muted-foreground))] mb-1">
                    ...
                    <input
                      ...
                      max={36}
                      value={sigTextSize}
                      onChange={(e) => setSigTextSize(Number(e.target.value))}
                      className="w-full px-2 py-1.5 text-sm rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                    />
                  </div>
                </div>
      ...
      {status === "complete" && result && (
        <div
          className="p-4 rounded-[var(--radius-md)] bg-green-50 border border-green-200 text-green-700 dark:bg-green-950 dark:border-green-800 dark:text-green-300"
          role="status"
        >
