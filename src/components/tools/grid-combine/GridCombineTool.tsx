...
} from "@/lib/pdf/processors/grid-combine";
...
export function GridCombineTool({ className = "" }: GridCombineToolProps) {
  ...
  const [gridLayout, setGridLayout] =
    useState<GridCombineOptions["gridLayout"]>("2x2");
  const [pageSize, setPageSize] =
    useState<GridCombineOptions["pageSize"]>("A4");
  const [orientation, setOrientation] =
    useState<GridCombineOptions["orientation"]>("landscape");
  ...
  const [fillMode, setFillMode] =
    useState<GridCombineOptions["fillMode"]>("leave-empty");
  const [pageMode, setPageMode] =
    useState<GridCombineOptions["pageMode"]>("first-page-only");
  ...
  const generateThumbnail = useCallback(
    async (
      file: File,
    ): Promise<{ thumbnail: string; pageCount: number } | null> => {
      try {
  ...
  const handleCombine = useCallback(async () => {
    if (files.length < 2) {
      setError(
        tTools("gridCombine.minFilesError") ||
  ...
  return (
    <div className={`space-y-6 ${className}`.trim()}>
      ...
      <FileUploader
        ...
        description={
          tTools("gridCombine.uploadDescription") ||
        ...
        >
          ...
          {errorDetails && (
            <details className="mt-2">
              ...
              <pre className="mt-2 p-2 text-xs bg-red-100 rounded overflow-x-auto whitespace-pre-wrap break-all max-h-40 overflow-y-auto">
                {errorDetails}
              </pre>
            </details>
          )}
        ...
        <Card variant="outlined" className="bg-blue-50 border-blue-200">
          <div className="flex items-center gap-2 text-blue-800">
            ...
            <p className="text-sm font-medium">
              {pageMode === "all-pages"
                ? tTools("gridCombine.previewInfoAllPages", {
                    fileCount: files.length,
                    totalPages: totalSourcePages,
                    layout: gridLayout,
                    outputPages,
                  })
                : tTools("gridCombine.previewInfoFirstPage", {
                    fileCount: files.length,
                    layout: gridLayout,
                    outputPages,
                  })}
            ...
            <Card variant="outlined" size="lg">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-[hsl(var(--color-foreground))]">
                  {tTools("gridCombine.filesTitle") || "Files to Combine"} (
                  {files.length})
                </h3>
                <Button
                  variant="ghost"
              ...
              <p className="text-sm text-[hsl(var(--color-muted-foreground))] mb-4">
                {tTools("gridCombine.reorderHint") ||
              ...
              >
                {files.map((file, index) => (
                  ...
                  >
                    ...
                    <div className="flex-shrink-0 w-12 h-16 rounded overflow-hidden bg-[hsl(var(--color-muted))] flex items-center justify-center relative">
                      {file.thumbnail ? (
                        <Image
                          src={file.thumbnail}
                          alt={file.file.name}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[hsl(var(--color-muted-foreground))]">
                        ...
                        {file.pageCount &&
                          ` • ${file.pageCount} ${file.pageCount === 1 ? tTools("gridCombine.page") || "page" : tTools("gridCombine.pages") || "pages"}`}
                      </p>
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setGridLayout(
                        e.target.value as GridCombineOptions["gridLayout"],
                      )
                    }
                    disabled={isProcessing}
                  ...
                  >
                    <option value="1x2">
                      1×2 (2 {tTools("gridCombine.cells") || "cells"})
                    </option>
                    <option value="2x1">
                      2×1 (2 {tTools("gridCombine.cells") || "cells"})
                    </option>
                    <option value="2x2">
                      2×2 (4 {tTools("gridCombine.cells") || "cells"})
                    </option>
                    <option value="2x3">
                      2×3 (6 {tTools("gridCombine.cells") || "cells"})
                    </option>
                    <option value="3x2">
                      3×2 (6 {tTools("gridCombine.cells") || "cells"})
                    </option>
                    <option value="3x3">
                      3×3 (9 {tTools("gridCombine.cells") || "cells"})
                    </option>
                    <option value="4x4">
                      4×4 (16 {tTools("gridCombine.cells") || "cells"})
                    </option>
                  </select>
                  ...
                  <div className="mt-3 p-3 bg-[hsl(var(--color-muted)/0.3)] rounded-[var(--radius-md)]">
                            ...
                            {displayFile?.thumbnail ? (
                              <div className="relative w-full h-full">
                                <Image
                                  ...
                                  unoptimized
                                  sizes="64px"
                                  className={`object-contain ${!file && repeatFile ? "opacity-50" : ""}`}
                                />
                              </div>
                            ) : displayFile ? (
                              <span
                                className={`text-[hsl(var(--color-primary))] ${!file && repeatFile ? "opacity-50" : ""}`}
                              >
                    ...
                    <p className="text-xs text-center text-[hsl(var(--color-muted-foreground))] mt-2">
                      {cols} {tTools("gridCombine.columns") || "columns"} ×{" "}
                      {rows} {tTools("gridCombine.rows") || "rows"}
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setPageMode(
                        e.target.value as GridCombineOptions["pageMode"],
                      )
                    }
                    disabled={isProcessing}
                    className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                  >
                    <option value="first-page-only">
                      {tTools("gridCombine.firstPageOnly") || "First page only"}
                    </option>
                    <option value="all-pages">
                      {tTools("gridCombine.allPages") || "All pages"}
                    </option>
                  </select>
                  <p className="text-xs text-[hsl(var(--color-muted-foreground))] mt-1">
                    {pageMode === "all-pages"
                      ? tTools("gridCombine.allPagesHint") ||
                        "Include all pages from each PDF in the grid."
                      : tTools("gridCombine.firstPageOnlyHint") ||
                        "Only use the first page from each PDF."}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFillMode(
                        e.target.value as GridCombineOptions["fillMode"],
                      )
                    }
                    disabled={isProcessing}
                    className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                  >
                    <option value="leave-empty">
                      {tTools("gridCombine.leaveEmpty") || "Leave empty cells"}
                    </option>
                    <option value="repeat">
                      {tTools("gridCombine.repeat") || "Repeat from start"}
                    </option>
                    <option value="stretch-last">
                      {tTools("gridCombine.stretchLast") || "Repeat last page"}
                    </option>
                  </select>
                  <p className="text-xs text-[hsl(var(--color-muted-foreground))] mt-1">
                    {tTools("gridCombine.fillModeHint") ||
                      "How to fill empty cells when files are fewer than grid cells."}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setPageSize(
                        e.target.value as GridCombineOptions["pageSize"],
                      )
                    }
                    disabled={isProcessing}
                    className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setOrientation(
                        e.target.value as GridCombineOptions["orientation"],
                      )
                    }
                    disabled={isProcessing}
                    className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                  >
                    <option value="landscape">
                      {t("options.landscape") || "Landscape"}
                    </option>
                    <option value="portrait">
                      {t("options.portrait") || "Portrait"}
                    </option>
                  </select>
                </div>
                ...
                <div className="space-y-2 pt-2 border-t border-[hsl(var(--color-border))]">
                  ...
                  {addBorder && (
                    <div className="pl-6">
                      <label
                        htmlFor="borderColor"
                        className="block text-xs text-[hsl(var(--color-muted-foreground))] mb-1"
                      >
      ...
      {status === "complete" && result && (
        ...
        >
          <p className="text-sm font-medium">
            {tTools("gridCombine.successMessage") ||
              "PDFs combined successfully! Click the download button to save your file."}
          </p>
        </div>
      )}
    </div>
