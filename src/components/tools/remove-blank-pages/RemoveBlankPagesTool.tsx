...
export function RemoveBlankPagesTool({
  ...
  const [filterMode, setFilterMode] = useState<"all" | "blanks" | "keeps">(
    "all",
  );
  ...
  const isPageRemoved = useCallback(
    (page: PageInfo) => {
  ...
  const loadAndAnalyzePdf = async (selectedFile: File) => {
    ...
    try {
      ...
      console.error(err);
      setError(
        err instanceof Error ? err.message : "Failed to analyze PDF pages.",
      );
    } finally {
    ...
    setUserOverrides(
      (prev) =>
        ({
          ...
          [pageNumber]:
            !currentlyRemoved === autoIsBlank ? undefined : !currentlyRemoved,
  ...
  return (
    <div className={`space-y-6 ${className}`.trim()}>
      ...
      {isAnalyzing && (
        <Card
          variant="outlined"
          className="p-8 border border-white/20 dark:border-zinc-800/40 bg-white/40 dark:bg-black/30 backdrop-blur-md rounded-3xl text-center space-y-4"
        >
      ...
      {file && !isAnalyzing && status !== "complete" && (
        <div className="space-y-6">
          ...
          <Card
            variant="outlined"
            className="p-4 flex flex-wrap items-center justify-between border border-zinc-200 dark:border-zinc-800/80 bg-white/50 dark:bg-zinc-950/20 backdrop-blur-sm rounded-2xl gap-4"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p
                  className="font-semibold text-sm text-[hsl(var(--color-foreground))] truncate max-w-[280px]"
                  title={file.name}
                >
                  {file.name}
                </p>
                <p className="text-xs text-[hsl(var(--color-muted-foreground))]">
                  {t("removeBlankPages.pagesCount", {
                    count: pages.length,
          ...
          <Card
            variant="default"
            className="p-6 border border-zinc-200/60 dark:border-zinc-800/40 bg-white/50 dark:bg-zinc-900/30 backdrop-blur-md rounded-[2rem] shadow-xl space-y-6"
          >
              ...
              <div className="md:col-span-5 flex flex-col justify-between border-t md:border-t-0 md:border-l border-zinc-200 dark:border-zinc-800 pt-6 md:pt-0 md:pl-6 space-y-4">
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2.5 pt-1">
                    ...
                    <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400">
                      {t("removeBlankPages.keepLabel", {
                        count: keepPagesCount,
                      })}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={handleSelectAllAsBlank}
            ...
            <div className="border-t border-zinc-100 dark:border-zinc-800/80 pt-4 flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {isProcessing
                    ? t("status.processing")
                    : tTools("removeBlankPages.removeButton")}
                </Button>
              </div>
            </div>
          </Card>
          ...
          {displayedPages.length === 0 ? (
            <Card
              variant="outlined"
              className="p-12 text-center rounded-[2rem] border-dashed border-zinc-200 dark:border-zinc-800"
            >
              ...
              <p className="text-sm text-[hsl(var(--color-muted-foreground))] font-bold">
                {t("removeBlankPages.noPagesToDisplay") ||
                  "No pages to display"}
              </p>
            </Card>
          ) : (
      ...
      {status === "complete" && result && (
        <Card
          variant="default"
          className="p-8 rounded-[2.5rem] bg-white/40 dark:bg-black/30 backdrop-blur-md border border-white/20 dark:border-zinc-800/40 text-center space-y-6 shadow-2xl"
        >
          ...
          <div className="flex gap-3 justify-center max-w-xs mx-auto">
            <DownloadButton
              file={result}
              filename={
                file?.name.replace(".pdf", "_no_blanks.pdf") ||
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          aspectRatio: page.aspectRatio ? `${page.aspectRatio}` : "0.707",
        }}
      >
        <Image
          src={page.dataUrl}
          ...
          className={`object-contain rounded shadow-sm select-none pointer-events-none transition-transform duration-500 ${
            isRemoved
              ? "filter grayscale opacity-30 scale-[0.98]"
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-200 z-20 pointer-events-none">
          <span className="text-[10px] font-bold text-white bg-black/60 px-2.5 py-1 rounded-full backdrop-blur-md shadow-md border border-white/10 uppercase tracking-wider">
            {isRemoved
              ? t("removeBlankPages.clickToKeep")
              : t("removeBlankPages.clickToDelete")}
          </span>
        </div>
      </div>
      ...
      <div
        className={`p-2 flex items-center justify-between text-[11px] font-bold border-t ${
          isRemoved
      ...
      >
        ...
        {/* Entropy Tag */}
        <span
          className="text-[9px] font-medium text-zinc-400 font-mono tracking-tighter"
          title={`Shannon Entropy: ${page.entropy.toFixed(4)}`}
        >
