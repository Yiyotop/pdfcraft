...
import {
  ...
  DPI_PRESETS,
} from "@/lib/pdf/processors/rasterize";
...
export function RasterizePDFTool({ className = "" }: RasterizePDFToolProps) {
  ...
  const handleRasterize = useCallback(async () => {
    ...
    try {
      ...
      const output = await rasterizePDF(file, options, (prog) =>
        setProgress(prog),
      );
      ...
      if (output.success && output.result) {
        ...
        setResultFilename(
          output.filename ||
            (format === "pdf" ? "rasterized.pdf" : "images.zip"),
        );
      } else {
  ...
  return (
    <div className={`space-y-6 ${className}`.trim()}>
      ...
      <FileUploader
        ...
        description={
          tTools("rasterizePdf.uploadDescription") ||
          "Drag and drop a PDF file to convert to images."
        }
      />
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {(["png", "jpeg", "webp", "pdf"] as RasterizeFormat[]).map(
                  (fmt) => (
                    <button
                      key={fmt}
                      type="button"
            ...
            <div>
              <label className="block text-sm font-medium text-[hsl(var(--color-foreground))] mb-2">
                {tTools("rasterizePdf.pageRangeLabel") ||
                  "Page Range (optional)"}
              </label>
              <input
                type="text"
              ...
              <p className="mt-1 text-xs text-[hsl(var(--color-muted-foreground))]">
                {tTools("rasterizePdf.pageRangeDesc") ||
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium">
              {tTools("rasterizePdf.successMessage") ||
                "Images created successfully! Click download to save."}
            </p>
          </div>
        </div>
      )}
